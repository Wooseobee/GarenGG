...
public class SaveMatchService {
    ...
    @Async("taskExecutor")
    public void saveMatch(String matchId, String tier, String rank, int usedKey) throws InterruptedException {
        ...
        if (curKey == usedKey) curKey = key.getAndIncrement() % API_KEY.size();
        String apiKey = API_KEY.get(curKey);
...
//                    log.info("used key : {}", curKey);
                    return;
                }
            } catch (Exception e) {
                log.error("매치 경기 API 요청 실패: key {}", curKey);
                attempts++;
